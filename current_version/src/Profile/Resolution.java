...
public class Resolution 
{
	...
	public volatile LinkedList<String> list_ioc = null;
	...
	public Resolution(String addr, SOURCE source)
	{
		try
		{
			...
			//
			if(!this.is_private_non_routable_address && !GEO_Location.TREE_GEO_LOCATION.containsKey(address_normalized) && !GEO_Location.TREE_NOT_FOUND.containsKey(address_normalized) && GEO_Location.AUTOMATIC_GEO_RESOLUTION_ENABLED)
			{
	...
	public boolean request_to_resolve(SOURCE source, String request)
	{
		try
		{
			...
			//Check GEO Location - address_normalized
			//
			/*if(!this.is_private_non_routable_ip(request) && GEO_Location.AUTOMATIC_GEO_RESOLUTION_ENABLED && !GEO_Location.TREE_GEO_LOCATION.containsKey(request) && !GEO_Location.TREE_NOT_FOUND.containsKey(request))
			{
				//neither tree has the dst ip address, request for resolution
	...
	public String get_map_details(boolean include_map_header)
	{
		try
		{
			...
			value_map = value_map + "<b>Source Address</b>:&nbsp" + address + "<br>";
			
			if(domain_name != null && !domain_name.trim().equals(""))
				value_map = value_map + "<b>Domain Name</b>:&nbsp&nbsp&nbsp&nbsp" + domain_name + "<br>";
			
			if(list_dns_response_addresses != null && list_dns_response_addresses.size() > 0)
			{
				value_map = value_map + "<b>IP Address</b>: " + "<br>";
				
				for(String ip : list_dns_response_addresses)
				{
					value_map = value_map + "&nbsp&nbsp&nbsp&nbsp" + ip + "<br>";
				}
			}
			
			value_map = value_map + "<br>";
			
			value_map = value_map + "<b>First Contact Time</b>: " + this.first_contact_time + "<br>";
			value_map = value_map + "<b>Last Contact Time</b>: " + this.last_link_time + "<br>";
			value_map = value_map + "<b>Source Link Cardinality</b>: " + this.tree_source.size() + "<br>";
			
			value_map = value_map + "<br>";
			
			if(alert_indicator != null && !alert_indicator.trim().equals(""))
			{
				value_map = value_map + "<b>ALERT DETECTED ON THIS RESOURCE!</b><br>";
			}
			...
			if(this.tree_source != null && this.tree_source.size() > 0)
			{
				value_map = value_map + "<b>Requesting (Source) Nodes</b>: " + "<br>";
				...
				{										
					...
					value_map = value_map + "&nbsp&nbsp&nbsp&nbsp" + node.src_ip + "&nbsp&nbsp&nbsp&nbsp" +  node.src_mac + "&nbsp&nbsp&nbsp&nbsp" + node.oui + "<br>";
