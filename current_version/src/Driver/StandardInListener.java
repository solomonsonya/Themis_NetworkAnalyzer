...
public class StandardInListener extends Thread implements Runnable
{
	...
	public boolean determineCommand(String line)
	{
		try
		{
			...
			else if(lower.startsWith("connect_gaius" )|| lower.startsWith("connect gaius") || lower.startsWith("gaius_connect" )|| lower.startsWith("gaius connect"))
				GEO_Location.connect_to_gaius(lower.substring("connect_gaius".length()));
			
			else if(lower.startsWith("connect_geo" )|| lower.startsWith("connect geo") || lower.startsWith("geo_connect" )|| lower.startsWith("geo connect"))
				GEO_Location.connect_to_gaius(lower.substring("connect_geo".length()));			
						
			else if(lower.startsWith("set_gaius_address") || lower.startsWith("set gaius address"))
				GEO_Location.set_gaius_address(lower.substring("set_gaius_address".length()));
			
			else if(lower.startsWith("set_gaius_ip") || lower.startsWith("set gaius ip"))
				GEO_Location.set_gaius_address(lower.substring("set_gaius_ip".length()));
			
			else if(lower.startsWith("set_geo_address") || lower.startsWith("set geo address"))
				GEO_Location.set_gaius_address(lower.substring("set_geo_address".length()));
			
			else if(lower.startsWith("set_geo_ip") || lower.startsWith("set geo ip"))
				GEO_Location.set_gaius_address(lower.substring("set_geo_ip".length()));
			
			else if(lower.startsWith("set_gaius") || lower.startsWith("set gaius"))
				GEO_Location.set_gaius_address(lower.substring("set_gaius".length()));
			
			else if(lower.startsWith("set_geo") || lower.startsWith("set geo"))
				GEO_Location.set_gaius_address(lower.substring("set_geo".length()));
			...
			else if(line.toLowerCase().startsWith("excalibur_ids_connect") || line.toLowerCase().startsWith("excalibur ids connect"))
				resolution_connect(line.substring(21));
			
			else if(line.toLowerCase().startsWith("ids_connect") || line.toLowerCase().startsWith("ids connect"))
				resolution_connect(line.substring(11));
			
			else if(line.toLowerCase().startsWith("connect_ids") || line.toLowerCase().startsWith("connect ids"))
				resolution_connect(line.substring(11));
	...
	public static boolean set_encryption(String key)
	{
		...
		try
		{
			...
			for(Whois_IDS_ResolutionRequest_ThdSocket thd : Whois_IDS_ResolutionRequest_ThdSocket.ALL_CONNECTIONS)
			{
				try
				{
					if(key == null || key.trim().equals(""))
	...
	public boolean resolution_connect(String location)
	{
		try
		{
			...
			//Connect
			driver.directive("Attempting to connect out to IDS Whois resolution request server --> " + address + " : " + port);
	...
	public boolean display_status()
	{
		try
		{
			...
			if(driver.my_external_ip_address != null && !driver.my_external_ip_address.trim().equals(""))
				driver.directive("External IP Address: " + driver.my_external_ip_address);
			
			if(GEO_Location.GLOBAL_GAIUS_CONNECTION_ADDRESS != null)
				driver.directive("Gaius Address: " + GEO_Location.GLOBAL_GAIUS_CONNECTION_ADDRESS + " : " + GEO_Location.GLOBAL_GAIUS_CONNECTION_PORT);
			
			if(Parser.GLOBAL_EXCALIBUR_IDS_CONNECTION_ADDRESS != null)
				driver.directive("Excalibur IDS Address: " + Parser.GLOBAL_EXCALIBUR_IDS_CONNECTION_ADDRESS + " : " + Parser.GLOBAL_EXCALIBUR_IDS_CONNECTION_PORT);
			...
			if((Whois_IDS_ResolutionRequest_ServerSocket.list_server_sockets != null && !Whois_IDS_ResolutionRequest_ServerSocket.list_server_sockets.isEmpty()))
			{
				for(Whois_IDS_ResolutionRequest_ServerSocket svrskt : Whois_IDS_ResolutionRequest_ServerSocket.list_server_sockets)
				{
					driver.directive("IDS Whois Request Resolution ServerSocket --> " + svrskt.get_status());
				}
			}
			
			if(Whois_IDS_ResolutionRequest_ThdSocket.list_outbound_connections != null && !Whois_IDS_ResolutionRequest_ThdSocket.list_outbound_connections.isEmpty())
			{
				driver.directive("Num Outbound Whois IDS Socket connections: " + Whois_IDS_ResolutionRequest_ThdSocket.list_outbound_connections.size());
				
				for(Whois_IDS_ResolutionRequest_ThdSocket thd : Whois_IDS_ResolutionRequest_ThdSocket.list_outbound_connections)
				{
					driver.directive("\tOutbound Whois IDS Socket -->" + thd.CONNECTION_ADDRESS);
				}
			}
