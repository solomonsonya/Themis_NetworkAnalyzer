...
public class JTable_Solomon extends JPanel implements ItemListener, ActionListener, ListSelectionListener, MouseListener
{
	...
	public volatile JTextArea_Solomon jta_data = new JTextArea_Solomon("", false, "", false);		
	...
	public volatile boolean process_double_click_for_netstat_table = false;
	public volatile boolean process_double_click_for_process_table = false;
	public volatile boolean process_double_click_for_typed_urls = false;
	public volatile boolean process_double_click_for_geo = false;
	...
	public boolean initializeGUI(Vector vColNm)
	{
		try
		{
			...
			this.jpnlNotificationPane.add(BorderLayout.CENTER, jta_data);
			
			
			
			this.jpnlNotificationPane.add(BorderLayout.SOUTH, jpnlNotification_South);
			
			jpnlJCB.add(BorderLayout.WEST, this.jta_data.jcbAutoScroll);
			jpnlJCB.add(BorderLayout.EAST, this.jta_data.jcbRejectUpdate);
			jpnlNotification_South.add(this.jpnlJCB);
			jpnlNotification_South.add(this.jta_data.jbtnExportData);
			...
			jta_data.jcbAutoScroll.setSelected(false);
			...
			this.jta_data.restrict_data_entries = false;
...
public String hash_sha256(String strToHash) throws Exception {	return this.sha256Hash(strToHash);	}
	...
	public boolean displaySelectedRowInDataView(int rowSelected)
	{
		try
		{
			...
			this.jta_data.append(displayString);
