...
{	
	...
	public static volatile boolean AUTOMATIC_GEO_RESOLUTION_ENABLED = true;
	...
	public boolean process_line(String line)
	{
		try
		{
			...
			try
			{
				...
				Resolution rsrc = null;
				
				//
				//iterate through the sources that have communicated with this resource
				//
				for(SOURCE src : SOURCE.TREE_SOURCE_NODES.values())
				{
					...
					if(src.tree_my_domain_name_requests.containsKey(this.address) )
					{
						rsrc = src.tree_my_domain_name_requests.get(this.address);
						
						if(rsrc == null || rsrc.geo != null)
							continue;
						
						rsrc.geo = this;
					}
					
					if(src.tree_my_dst_ip.containsKey(this.address) )
					{
						rsrc = src.tree_my_dst_ip.get(this.address);
				...
				//
				SOURCE src = null;
				for(Resolution resolution : Resolution.TREE_RESOURCE.values())
				{
					if(resolution == null)
						continue;
					
					if(resolution.tree_source.containsKey(this.address) )
					{
						src = resolution.tree_source.get(this.address);
						
						if(src == null || src.geo != null)
							continue;
	...
	public static boolean update_geo_resolution()
	{
		try
		{
			...
			if(SOURCE.TREE_SOURCE_NODES == null || SOURCE.TREE_SOURCE_NODES.isEmpty() || Resolution.TREE_RESOURCE == null || Resolution.TREE_RESOURCE.isEmpty())
				return false;
			...
			for(Resolution rsrc : Resolution.TREE_RESOURCE.values())
			{
				...
				else if(TREE_NOT_FOUND.containsKey(rsrc.address))
					continue;
						
				
				driver.directive("g " + rsrc.address);
				//otw, resolve!
